...
const ProjectDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (router.isReady && projectId) {
      // Fix: Cast projectId to string for type-safe comparison.
      const foundProject = projects.find(p => p.id === (projectId as string)) || null;
      setProject(foundProject);
    }
  }, [router.isReady, projectId]);

  if (!project) {
    if (!router.isReady) return <div className="text-center py-20">Loading project...</div>;
    return (
        <div className="text-center py-20">
          ...
          <Link href="/portfolio" className="mt-8 inline-block px-6 py-3 bg-accent text-background font-bold rounded-lg shadow-lg hover:bg-accent/80 transition-colors duration-300">
            Back to Portfolio
          </Link>
        </div>
    );
  ...
  return (
      <div className="max-w-5xl mx-auto">
        <Link href="/portfolio" className="text-accent hover:underline mb-8 inline-block">&larr; Back to Portfolio</Link>
